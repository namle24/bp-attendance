...
let bootstrap,csrf='',current,qrInfo,serverOffset=0,busy=false,renderedToken='',dashboardPending=false,qrPending=false;
...
function controls(){if(busy)return;$('close').disabled=!live();$('project').disabled=!current;$('copy-qr').disabled=!qrInfo||!live()||qrInfo.expiresAt<=Date.now()+serverOffset;}
async function start(){
  bootstrap=await api('/api/bootstrap');csrf=bootstrap.csrf;serverOffset=bootstrap.serverTime-Date.now();
  $('demo-banner').hidden=!bootstrap.demo;$('login').hidden=!!bootstrap.user;$('logout').hidden=!bootstrap.user;
  if(!bootstrap.user){
    $('demo-login').hidden=!bootstrap.demo;
    if(!bootstrap.demo){
  ...
  else{$('student').hidden=false;$('student-name').textContent=bootstrap.user.name;$('student-account').textContent=bootstrap.user.studentId+' · '+bootstrap.user.email;$('demo-paste').hidden=!bootstrap.demo;studentTick();await myAttendance();}
}
async function dashboard(){
  ...
  $('roster-count').textContent=data.students+' sinh viên';
  $('sync-state').textContent=!data.sync.enabled?'Demo: không ghi Google Sheets':data.sync.error?'Sheets đang lỗi; bản ghi đã lưu, sẽ thử lại':data.sync.pending?'Đã lưu · đang chờ đồng bộ Sheets':data.sync.lastSync?'Đã đồng bộ Sheets lúc '+time(data.sync.lastSync):'Chưa có dữ liệu đồng bộ';
  ...
  for(const id of ['session-date','online-date','correction-date'])if(!$(id).value)$(id).value=data.today;
...
function studentTick(){
  ...
  $('check-in').hidden=completed;
  $('check-in').disabled=busy||!token||remaining<=0;
  $('student-instruction').textContent=completed?'Điểm danh đã lưu. Bạn có thể đóng trang.':!token?'Quét QR đang chiếu trong phòng để điểm danh.':remaining>0?'Đã nhận QR. Bấm xác nhận trước khi mã hết hạn.':'QR đã hết hạn. Hãy quét lại mã mới đang chiếu; bạn vẫn đang đăng nhập.';
...
$('demo-admin').addEventListener('click',()=>action(async()=>{await api('/api/demo/login',{kind:'admin'});location.reload();}));
$('demo-student').addEventListener('click',()=>action(async()=>{await api('/api/demo/login',{kind:'student'});location.reload();}));
$('logout').addEventListener('click',()=>action(async()=>{await api('/api/logout',{});location.reload();}));
$('check-in').addEventListener('click',()=>action(async()=>{
  let data;
  try{data=await api('/api/check-in',{token});}
  catch(error){
...
$('use-link').addEventListener('click',()=>{try{token=new URL($('pasted-link').value).hash.slice(1);sessionStorage.setItem('bp-qr',token);studentTick();}catch{notice('Link không hợp lệ.',true);}});
$('session').addEventListener('change',()=>{qrInfo=null;action(async()=>{await dashboard();notice('');});});
$('open').addEventListener('click',()=>action(async()=>{await api('/api/admin/sessions',{date:$('session-date').value,minutes:Number($('duration').value)});$('session').value='';await dashboard();notice('Đã mở phiên. Chỉ chiếu QR trong phòng học.');}));
...
$('project').addEventListener('click',()=>{document.body.classList.add('projection');$('exit-project').hidden=false;});
...
$('sync').addEventListener('click',()=>action(async()=>{const r=await api('/api/admin/sync',{});await dashboard();notice(r.sync.error||(!r.sync.enabled?'Demo không ghi Sheets.':r.sync.pending?'Đã lưu, còn thay đổi chờ đồng bộ.':'Đã đồng bộ Sheets.'),!!r.sync.error);}));
