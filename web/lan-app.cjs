...
function createAdminApp(config,store,worker,options={}){
  ...
  app.get('/api/export.csv',(req,res)=>res.type('text/csv').attachment('BP_Attendance.csv').send(store.csv()));
  app.get('/api/detail.csv',(req,res)=>res.type('text/csv').attachment('BP_Offline_Check.csv').send(store.detailCSV()));
