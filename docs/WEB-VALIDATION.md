...
On 2026-09-18, [Pages deployment 35326630669](https://github.com/namle24/bp-attendance/actions/runs/35326630669) successfully published revision `faca919`. The live HTML, JavaScript and CSS returned HTTP 200 and matched the repository files. Running `scripts/test-location-ui.cjs` with `BP_LOCATION_LIVE_HELPER=1` passed the same location scenarios against the actual HTTPS site without response interception; GPS remained simulated. The existing full browser regression scenario also passed again. Physical phone permissions/location and classroom network load remain unverified for this feature.
