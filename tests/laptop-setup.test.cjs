...
test('laptop preparation keeps existing secrets, config and database when run again',()=>{
  ...
  try{
    ...
    assert.equal(a.PUBLIC_ORIGIN,'');assert.equal(a.CAMPUS_CIDRS,'');assert.equal(a.GOOGLE_CLIENT_ID,'');
    ...
    const report=inspect(dir);assert.equal(report.ready,false);
    assert.ok(report.checks.find(c=>c.name==='QR_SECRET').ok);
    assert.equal(report.checks.find(c=>c.name==='Chứng chỉ HTTPS và private key').ok,false);
