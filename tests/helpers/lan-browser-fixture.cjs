...
const {StudentLookup}=require('../../web/student-lookup.cjs');
...
async function startFixture(){
  ...
  const start=app=>new Promise(resolve=>{const server=require('../../web/listener.cjs').listen(app,0,'127.0.0.1',()=>resolve(server));});
  const student=await start(createStudentApp(config,store,{lookup})),admin=await start(createAdminApp(config,store,worker,{lookup}));
