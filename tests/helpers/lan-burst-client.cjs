...
async function main(){
  ...
  const scans=await Promise.allSettled(bodies.map(async body=>{
    ...
    assert.equal(response.status,200);body.cookie=response.headers.get('set-cookie').split(';')[0];body.scanTicket=(await response.json()).scanTicket;
  }));
  ...
  for(const duplicate of [false,true]){
    const results=await Promise.allSettled(bodies.map(async body=>{
      const response=await fetch(origin+'/api/check-in',{method:'POST',headers:{Origin:origin,'Content-Type':'application/json',Cookie:body.cookie},body:JSON.stringify(body),signal:AbortSignal.timeout(30000)});
