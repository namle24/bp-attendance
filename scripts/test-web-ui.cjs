...
(async()=>{
  ...
  try{
    ...
    assert.equal(fixture.store.sessions().length,1);await admin.setViewportSize({width:1280,height:720});
    assert.ok(await admin.evaluate(()=>document.documentElement.scrollHeight<=innerHeight),'QR and room code must fit a 720p projector without scrolling');
    await admin.screenshot({path:'docs/web-projector.png',fullPage:true});await admin.setViewportSize({width:1280,height:960});
